...
export default function BusinessProfile() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans text-slate-700">
      ...
      <span className="bg-emerald-100 text-emerald-700 text-sm font-semibold px-3 py-1 rounded-full border border-emerald-200">        {client.status}
      </span>
     </div>
...
<div className="flex items-center bg-gray-100/80 p-1.5 rounded-xl mb-8 w-fit border border-gray-200 shadow-sm">
  {tabs.map((tab) => {
    const handleClick = () => {
      switch (tab) {
        case "Orders & Quotes":
          router.push(`/dashboard/b2bCustomers/${id}/orders`);
          break;
        case "Financial Ledger":
          router.push(`/dashboard/b2bCustomers/${id}/ledger`);
          break;
        case "Custom Pricing":
          router.push(`/dashboard/b2bCustomers/${id}/customPricing`);
          break;
        case "Referral Programs":
          router.push(`/dashboard/b2bCustomers/${id}/referralPrograms`);
          break;
        default:
          setActiveTab(tab); 
      }
