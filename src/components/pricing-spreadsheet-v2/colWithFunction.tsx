...
export default function ColWithFunction(props: ColumnData) {
    ...
    const [units, setUnits] = useState<{
        ...
        selling_quantity: isUnits ? Number(columnData.selling_quantity) || 0 : 0
    });

    const [enableEdit, setEnableEdit] = useState(false);
    
    function handleSaveUnit() {
        if (isUnits && (columnFunctionType === "change-b2c_selling_unit" || columnFunctionType === "change-b2b_selling_unit")) {
            setEnableEdit(false);
            onChange({
                selling_unit: units.selling_unit,
                selling_quantity: Number(units.selling_quantity)
            });
        }
    }
        ...
        case "change-b2b_selling_unit":
            if (enableEdit)
                ...
                <div className="cursor-pointer text-sm text-center" onClick={() => setEnableEdit(true)}>
                    {columnData.selling_quantity + " " + columnData.selling_unit}
                </div>
            );
            
      // Dans tous les onChange pour les valeurs numériques, on force Number()
...
case "change-prix_sur_site":
    ...
    />;
        ...
        case "create-po":
            ...
            const poStatus = columnData.po_status as keyof typeof statusConfig | null;
            const commande = columnData.commande;
